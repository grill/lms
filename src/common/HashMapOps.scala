...
trait HashMapOpsExp extends HashMapOps with PointerExp with EffectExp with TupleOpsExp with FunctionsExp with ArrayOps with IfThenElseExp with EqualExp with SetOpsExp with LiftNumeric {
...
trait CLikeGenHashMapOps extends BaseGenHashMapOps with CLikeCodegen {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case m@HashMapExtendedNew(k,hf,v1,v2,ef) => {
        ...
        emitValDef(sym, "g_hash_table_new(" + quote(uhf) + "," + quote(uef) + ")")
    }
