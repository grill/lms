...
case _ => super.fresh
  ...
  case class SimpleStruct[T: Manifest](tag: StructTag[T], elems: Seq[(String, Rep[Any])]) extends AbstractStruct[T] {
    registerStruct(tag.asInstanceOf[ClassTag[_]].name, elems.map(e => (e._1, e._2.tp)))
  }
...
trait StructExpOpt extends StructExp {
  ...
  def fieldLookup[T](struct: Exp[Any], index: String): Option[Exp[T]] = {
    def lookup(elems: Seq[(String, Exp[Any])]) = elems.find(_._1 == index).getOrElse(
      throw new RuntimeException("ERROR: " + index + " is not a field of type " + struct.tp))._2.asInstanceOf[Exp[T]]
...
trait StructFatExpOptCommon extends StructFatExp with StructExpOptCommon with IfThenElseFatExp with BooleanOpsExp {
    ...
    case RecordEquals(r1, r2) =>
      sym.atPhase(LIRLowering) {
        ...
        def equals[T: Manifest](lrec1: Exp[T], lrec2: Exp[T]): Exp[Boolean] = {
          val fields = encounteredStructs(structName(manifest[T]))
          val e = var_new[Boolean](unit(true))
          fields.foreach { fld =>
            if (fld._2.erasure.isPrimitive || fld._2 == manifest[java.lang.Character])
              e = readVar(e) && __equal(field(lrec1, fld._1)(fld._2, implicitly[SourceContext]).asInstanceOf[Exp[A]], field(lrec2, fld._1)(fld._2, implicitly[SourceContext]).asInstanceOf[Exp[A]])
            else if (fld._2 == manifest[Array[Byte]]) {
              val fldtype = {
                val s = fresh(fld._2)
                s.tp = fld._2
              ...
              e = readVar(e) && __equal(a1.asInstanceOf[Exp[A]], arr2.asInstanceOf[Exp[A]])
