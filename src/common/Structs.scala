...
trait StructExpOptCommon extends StructExpOpt with VariablesExp with IfThenElseExp {
  ...
  override def var_assign[T:Manifest](lhs: Var[T], rhs: Exp[T])(implicit pos: SourceContext): Exp[Unit] = (lhs,rhs) match {
    case (Variable(Def(Struct(NestClassTag(tagL),elemsL: Map[String,Exp[Variable[Any]]] @unchecked))), Def(Struct(tagR, elemsR))) =>
      assert(tagL == tagR)
  ...
  override def readVar[T:Manifest](v: Var[T])(implicit pos: SourceContext): Exp[T] = v match {
    case Variable(Def(Struct(NestClassTag(tag), elems: Map[String,Exp[Variable[Any]]] @unchecked))) =>
...
trait BaseGenFatStruct extends GenericFatCodegen {
  ...
  override def fattenAll(e: List[Stm]): List[Stm] = {
    ...
    val r = e.flatMap {
      ...
      case TP(sym:Sym[Unit] @unchecked, o@IfThenElse(c,a:Block[Unit] @unchecked,b:Block[Unit] @unchecked)) => List(fatif(sym,o.asInstanceOf[Def[Unit]],c,a,b))
      case TP(sym:Sym[Unit] @unchecked, o@Reflect(IfThenElse(c,a:Block[Unit] @unchecked,b:Block[Unit] @unchecked),_,_)) => List(fatif(sym,o.asInstanceOf[Def[Unit]],c,a,b))
      case t => List(fatten(t))
    } ++ orphans.map { case s: Sym[Unit] @unchecked => fatphi(s).get } // be fail-safe here?
