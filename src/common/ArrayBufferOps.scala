...
trait ArrayBufferOpsExp extends ArrayBufferOps with EffectExp with VariablesExp with FunctionsExp {
  ...
  /*override def mirrorDef[A:Manifest](e: Def[A], f: Transformer)(implicit pos: SourceContext): Def[A] = (e match {
    ...
    case ArrayBufferSize(l) => ArrayBufferSize(f(l))
    case _ => super.mirrorDef(e,f)
  }).asInstanceOf[Def[A]] // why??*/
  
  override def mirror[A:Manifest](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case Reflect(a@ArrayBufferMkString(l,r), u, es) => reflectMirrored(Reflect(ArrayBufferMkString(f(l),f(r))(a.mA), mapOver(f,u), f(es)))(mtype(manifest[A]))
